...
public class QuakeEntry implements Comparable<QuakeEntry>{
    ...
    public int compareTo(QuakeEntry loc) {
        if(this.magnitude > loc.getMagnitude()) return 1;
        else if(this.magnitude < loc.getMagnitude()) return -1;
        else{
            if(this.depth > loc.getDepth()) return 1;
            else if(this.depth < loc.getDepth()) return -1;
            return 0;
        }   
